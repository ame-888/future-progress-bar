...
const statusLabel = (m: Measurement) => (m.valueStatus ?? "verified").replaceAll("-", " ").toUpperCase();
...
function MeasurementCard({ measurement }: { measurement: Measurement }) {
  ...
  return <article className="measurement-card" id={measurement.id}>
    ...
    <div className="measurement-result"><div><small>Observed current result</small><strong>{formatValue(measurement)} <span>· {statusLabel(measurement)}</span></strong><p>{measurement.dataPeriod ? `Data period: ${measurement.dataPeriod}` : `Checked ${formatDate(measurement.researchCutoff)}`}</p></div><div><small>Current level</small><strong>{LEVEL_NAMES[achieved]} · L{achieved}</strong><p>{next ? `Next: L${next.level} ${LEVEL_NAMES[next.level]} — ${next.label ?? `${next.goal.toLocaleString()} ${measurement.unit}`}` : "All seven scored levels reached"}</p></div></div>
    ...
    <div className="disclosures">
      <details><summary>Definition &amp; method</summary><p>{measurement.definition}</p>{measurement.methodNote && <p>{measurement.methodNote}</p>}<dl><div><dt>Temporal type</dt><dd>{measurement.temporalType}</dd></div><div><dt>Indicator</dt><dd>{measurement.indicatorType}</dd></div><div><dt>Unit</dt><dd>{measurement.unit}</dd></div></dl></details>
      <details><summary>Evidence {measurement.evidence.length ? `(${measurement.evidence.length})` : ""}</summary>{measurement.evidence.length ? measurement.evidence.map(source => <article key={source.url}><a href={source.url}>{source.organization ? `${source.organization}: ` : ""}{source.title}</a><p>{source.supports ?? "Supports the recorded current result."} Accessed {source.accessedDate}.</p></article>) : <p>Structured evidence metadata is incomplete for this result. No source has been fabricated.</p>}</details>
...
export function ProgressTable({ initialSubdomainId }: { initialSubdomainId?: string }) {
  ...
  return <div className="progress-detail" style={{ "--domain-accent": domain.id === "automation" ? "#ff5f6d" : domain.id === "hardware" ? "#f6c85f" : domain.id === "neuro" ? "#55b8ff" : domain.id === "sustainability" ? "#4dd9a7" : "#b58cff" } as React.CSSProperties}>
    ...
    {subdomain.northStar && <section className="north-star-panel"><div><p className="atlas-kicker">Contextual North Star — not part of the 336 scored milestones</p><h2>{subdomain.northStar.title}</h2><p>{subdomain.northStar.question}</p><small>{subdomain.northStar.methodology} · Data cutoff {formatDate(subdomain.northStar.lastUpdated)}</small></div>{Graph && <div className="north-star-chart"><Graph /></div>}</section>}

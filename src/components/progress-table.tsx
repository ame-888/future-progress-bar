...
export function ProgressTable({ initialSubdomainId }: { initialSubdomainId?: string }) {
  ...
  return <div className="progress-detail" style={{ "--domain-accent": domain.id === "automation" ? "#ff5f6d" : domain.id === "hardware" ? "#f6c85f" : domain.id === "neuro" ? "#55b8ff" : domain.id === "sustainability" ? "#4dd9a7" : "#b58cff" } as React.CSSProperties}>
    ...
    {subdomain.northStar && <section className="north-star-panel"><div><p className="atlas-kicker">Contextual North Star — not part of the 336 scored milestones</p><h2>{subdomain.northStar.title}</h2><p>{subdomain.northStar.question}</p><small>{subdomain.northStar.variable} · {subdomain.northStar.methodology} · Data cutoff {formatDate(subdomain.northStar.researchCutoff)} · definition v{subdomain.northStar.definitionVersion}. Provenance: {subdomain.northStar.series.provenance} Limitation: {subdomain.northStar.knownLimitations.join(" ")}</small></div>{Graph && <div className="north-star-chart"><Graph /></div>}</section>}

...
import { EvidenceBadge, FreshnessBadge, StatusBadge } from "./ui";
import { freshness } from "@/lib/transparency";
...
function MeasurementCard({ measurement }: { measurement: Measurement }) {
  ...
  return <article className="measurement-card" id={measurement.id}>
    <header><p className="atlas-kicker">Measurement {measurement.id}</p><h3>{measurement.title}</h3><p className="research-question">{measurement.question}</p><div className="badge-row"><StatusBadge status={measurement.valueStatus}/><FreshnessBadge state={freshness(measurement).state}/><EvidenceBadge count={measurement.evidence.length}/></div></header>
    ...
    <div className="disclosures">
      ...
      <p><strong>{measurement.evidence.length} evidence reference(s)</strong> · last checked {formatDate(measurement.researchCutoff)}</p>
      <Link className="forecast-link" href={`/research/${measurement.id}`}>Full specification →</Link>
...
export function ProgressTable({ initialSubdomainId }: { initialSubdomainId?: string }) {
  ...
  return <div className="progress-detail" style={{ "--domain-accent": domain.id === "automation" ? "#ff5f6d" : domain.id === "hardware" ? "#f6c85f" : domain.id === "neuro" ? "#55b8ff" : domain.id === "sustainability" ? "#4dd9a7" : "#b58cff" } as React.CSSProperties}>
    ...
    {subdomain.northStar && <section className="north-star-panel"><div><p className="atlas-kicker">Contextual North Star · non-scoring</p><h2>{subdomain.northStar.title}</h2><p>{subdomain.northStar.question}</p><div className="badge-row"><span className="badge badge--muted">Cutoff {formatDate(subdomain.northStar.researchCutoff)}</span><span className="badge badge--muted">Definition v{subdomain.northStar.definitionVersion}</span></div></div>{Graph && <div className="north-star-chart"><Graph /></div>}<details className="north-star-method"><summary>Methodology, provenance &amp; limitations</summary><p><strong>Variable:</strong> {subdomain.northStar.variable}</p><p>{subdomain.northStar.methodology}</p><p><strong>Provenance:</strong> {subdomain.northStar.series.provenance}</p><p><strong>Limitations:</strong> {subdomain.northStar.knownLimitations.join(" ")}</p></details></section>}

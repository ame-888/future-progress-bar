...
import { adsenseClient, adsenseEnabled } from "@/lib/site";
...
export function AdSlot({ slot, label = "Advertisement" }: { slot?: string; label?: string }) {
  useEffect(() => {
    if (!adsenseEnabled || !adsenseClient || !slot || !/^\d+$/.test(slot)) return;
    try {
  ...
  if (!adsenseEnabled || !adsenseClient || !slot || !/^\d+$/.test(slot)) return null;
